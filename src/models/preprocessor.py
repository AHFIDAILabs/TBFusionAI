...
class AudioPreprocessor:
    ...
    def load_audio(
        ...
        try:
            ...
            logger.error(f"Failed to load audio from {type(audio_input).__name__}: {str(e)}")
            raise ValueError(f"Failed to load audio from {type(audio_input).__name__}: {str(e)}")
    ...
    def validate_audio_quality(
        ...
        if metrics['duration'] < min_duration:
            return False, f"Audio too short ({metrics['duration']:.1f}s < {min_duration}s)", metrics
        
        if metrics['duration'] > max_duration:
            return False, f"Audio too long ({metrics['duration']:.1f}s > {max_duration}s)", metrics
        
        # Check SNR
        if metrics['snr'] < min_snr:
            return False, f"Poor audio quality (SNR {metrics['snr']:.1f}dB < {min_snr}dB). Please record in a quieter environment.", metrics
        
        # Check clipping
        if metrics['clipping_ratio'] > max_clipping_ratio:
            return False, f"Audio clipping detected ({metrics['clipping_ratio']*100:.1f}% > {max_clipping_ratio*100:.1f}%). Please reduce microphone volume.", metrics
        
        # Check silence
        if metrics['silence_ratio'] > max_silence_ratio:
            return False, f"Too much silence ({metrics['silence_ratio']*100:.1f}% > {max_silence_ratio*100:.1f}%). Please speak louder or closer to microphone.", metrics
    ...
    def extract_features(
        ...
        if validate_quality:
            ...
            logger.info(f"✓ Audio quality validated: SNR={metrics['snr']:.1f}dB, Duration={metrics['duration']:.1f}s")
