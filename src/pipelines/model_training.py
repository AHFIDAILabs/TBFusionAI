...
class ModelTrainingPipeline:
    ...
    async def _prepare_data(self, data_path: Optional[Any]) -> Tuple:
        ...
        logger.info(f"Training set:   {train_total:,} samples ({(1 - self.config.model_training.test_size) * 100:.0f}%)")
        logger.info(f"  - TB Positive: {train_pos:,} ({train_pos/train_total*100:.1f}%)")
        logger.info(f"  - TB Negative: {train_neg:,} ({train_neg/train_total*100:.1f}%)")
        ...
        logger.info(f"  - TB Positive: {valid_pos:,} ({valid_pos/valid_total*100:.1f}%)")
        logger.info(f"  - TB Negative: {valid_neg:,} ({valid_neg/valid_total*100:.1f}%)")
    ...
    async def _train_meta_cost_models(self, X_tr, X_val, y_tr, y_val):
        ...
        mc_log = LogisticRegression(C=0.1, penalty='l2', class_weight='balanced', max_iter=1000, random_state=42)
        ...
        mc_xgb = XGBClassifier(
            ...
            reg_alpha=0.1, reg_lambda=1.0, min_child_weight=5, scale_pos_weight=spw,
            eval_metric='logloss', random_state=42, early_stopping_rounds=50, verbosity=0)
        mc_xgb.fit(X_tr, y_tr, sample_weight=weights, eval_set=[(X_val, y_val)], verbose=False)
        ...
        mc_lgb = LGBMClassifier(
            n_estimators=500, max_depth=4, learning_rate=0.05, subsample=0.8, colsample_bytree=0.8,
            reg_alpha=0.1, reg_lambda=1.0, min_child_samples=20, scale_pos_weight=spw, random_state=42, verbose=-1)
        mc_lgb.fit(X_tr, y_tr, sample_weight=weights, eval_set=[(X_val, y_val)], eval_metric='binary_logloss',
        ...
        mc_cat = CatBoostClassifier(
            iterations=500, depth=4, learning_rate=0.05, l2_leaf_reg=3.0, subsample=0.8,
            colsample_bylevel=0.8, min_data_in_leaf=20, scale_pos_weight=spw, random_state=42,
            verbose=0, thread_count=-1, early_stopping_rounds=50)
        mc_cat.fit(X_tr, y_tr, sample_weight=weights, eval_set=(X_val, y_val), verbose=False)
        ...
        mc_mlp = AsymmetricLossMLPClassifier(
            fn_cost=10.0, fp_cost=1.0, hidden_layer_sizes=(100, 50), alpha=0.01,
    ...
    def _evaluate_model(self, name: str, model: Any, X_tr: np.ndarray, y_tr: pd.Series, 
        ...
        try:
            cv_recall = cross_val_score(model, X_tr, y_tr, cv=5, scoring='recall', n_jobs=-1)
            self.cv_scores[name] = {
    ...
    def _detect_overfitting(self):
        ...
        for name in self.model_metrics.keys():
            ...
            if train_m['false_negatives'] == 0 and train_m['false_positives'] == 0:
                overfitting_detected = True
                logger.warning(f"⚠️  {name}: Perfect training (FN=0, FP=0) - Still overfitting!")
        
        if not overfitting_detected:
            logger.info("✓ No significant overfitting detected - Regularization working!")
    ...
    async def _evaluate_and_select_models(self, y_valid: pd.Series) -> str:
        ...
        logger.info(f"   FN: {best_m['false_negatives']} | FP: {best_m['false_positives']}")
    ...
    async def _save_models_and_metadata(self, best_model_name: str):
        ...
        top_3 = sorted(self.model_metrics.items(), key=lambda x: x[1]['false_negatives'])[:3]
        ...
        for i, (name, m) in enumerate(top_3, 1):
            logger.info(f"  {i}. {name}: FN={m['false_negatives']}, FP={m['false_positives']}")
