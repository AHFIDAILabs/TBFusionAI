...
class DataIngestionPipeline:
    ...
    def __init__(self):
        ...
        self.raw_data_path = (
            self.dataset_path / self.config.data_ingestion.raw_data_subdir
        )
        self.meta_data_path = (
            self.dataset_path / self.config.data_ingestion.meta_data_subdir
        )
    ...
    async def _download_and_extract_dataset(self) -> None:
        ...
        if self.dataset_path.exists() and len(list(self.dataset_path.iterdir())) > 0:
            logger.info(
                f"Dataset already exists at {self.dataset_path}. Skipping download."
            )
            return
            ...
            try:
                if filepath.suffix == ".csv":
                    ...
                    try:
                        ...
                        logger.warning(
                            "Install openpyxl to read Excel files: pip install openpyxl"
                        )
                ...
                if key in critical_files:
                    logger.error(
                        f"Failed to load critical file {key} from {filepath}: {e}"
                    )
                    raise
                else:
                    logger.warning(
                        f"Failed to load optional file {key} from {filepath}: {e}"
                    )
        ...
        for critical_file in critical_files:
            if critical_file not in metadata_dfs:
                raise FileNotFoundError(
                    f"Critical metadata file not loaded: {critical_file}"
                )
