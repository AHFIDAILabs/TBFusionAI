"""
FastAPI dependencies for TBFusionAI - FIXED: WebM format support.

Provides:
- Predictor instance management with absolute path resolution
- Configuration management (cached)
- Request validation for audio files (including WebM)
...
async def validate_audio_file(audio_file: UploadFile) -> UploadFile:
    ...
    CRITICAL FIX: Added WebM format support for browser recording.
    """
    ...
    # CRITICAL FIX: Accept WebM files from browser recording
    allowed_formats = list(config.api.allowed_audio_formats) + [".webm"]
    ...
    logger.info(
        f"✓ File validated: {audio_file.filename} ({file_size} bytes, format: {file_ext})"
    )
    return audio_file
